...
class Spider(Spider):
    ...
    def _extractVideoItems(self, html):
        vods = []
        try:
            # 多模板兼容
            items = re.findall(
                r'<a[^>]*class="thumbnail"[^>]*href="([^"]+)"[^>]*>.*?<img[^>]*data-original="([^"]+)"[^>]*>.*?<span[^>]*class="title[^"]*"[^>]*>(.*?)</span>',
                html, re.S)
            if not items:
                items = re.findall(
                    ...
                    html, re.S)
                # 调整顺序： href, title, img
                items = [(href, title, img) for href, title, img in items]

            for href, title_raw, img in items:
    ...
    def homeContent(self, filter):
        classes = [
            ...
            {'type_id': '/index.php/vod/type/id/33.html', 'type_name': '中文字幕'},
    ...
    def detailContent(self, ids):
        ...
        vod = {'vod_id': vid, 'vod_name': '', 'vod_pic': '', 'vod_remarks': '', 'vod_content': ''}
        try:
            res = self.fetch(url, headers=self.headers)
            res.encoding = 'utf-8'
            html = res.text
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        try:
            ...
            video_url = None
            matches = re.findall(r'["\'](https?://[^"\']+\.(?:m3u8|mp4)[^"\']*)["\']', html)
            for m in matches:
                if '.html' not in m:
                    video_url = m
                    break
    ...
    def searchContent(self, key, quick, pg="1"):
        result = {'list': []}
        try:
            search_url = f"{self.host}/index.php/vod/search/wd/{key}/page/{pg}.html"
